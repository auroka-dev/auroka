...
behavior! {
  ...
  when_something_happens(Ordering::Equal)

  "Something is true" {
    then_something_should_be_true(Ordering::Equal)
  }
}"#,
  );
...
fn something_is_true() {
    ...
    when_something_happens(&mut context, Ordering::Equal);
    then_something_should_be_true(&context, Ordering::Equal);
}
"#,
  );
}
