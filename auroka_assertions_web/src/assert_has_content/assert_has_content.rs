...
#[doc(hidden)]
pub async fn assert_has_content_internal(url: &str, method: Option<&str>, expected: &str) -> Result<()> {
...
macro_rules! assert_has_content {
  ($base:expr, $path:expr, : $method:ident, $expected:expr) => {
    $crate::assert_has_content::assert_has_content_internal(&format!("{}{}", $base, $path), Some(stringify!($method)), $expected).await?
  };
  ($base:expr, $path:expr, $expected:expr) => {
    $crate::assert_has_content::assert_has_content_internal(&format!("{}{}", $base, $path), None, $expected).await?
  };
